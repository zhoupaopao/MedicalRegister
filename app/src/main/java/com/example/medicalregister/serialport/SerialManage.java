...
public class SerialManage {
    ...
    public void open() {
        ...
        //"/dev/ttyS1"
        isConnect = serialHandle.open("/dev/ttyS1", 9600, true);
        //serialHandle.open("/dev/ttyUSB4",8,1,0,2400,true);//设置地址，波特率，开启读取串口数据
        Log.e("TAG",isConnect+" 串口状态");
    }
    public void open(String devicePath, int baudrate, int dataBits, int stopBits, int parity, boolean isRead) {
        ///sys/bus/usb/devices/usb4
        //"/dev/ttyS1"
        isConnect = serialHandle.open(devicePath, baudrate,  dataBits,  stopBits,  parity,  isRead);
        //serialHandle.open("/dev/ttyUSB4",8,1,0,2400,true);//设置地址，波特率，开启读取串口数据
        Log.e("TAG",isConnect+" 串口状态");
    }
    /**
